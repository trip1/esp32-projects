...
def diagram(project: dict, target: dict, configuration: dict | None = None) -> str:
    ...
    warning_text = "".join(
        f'<text x="70" y="{790+i*27}" class="warning">⚠ {escape(warning)}</text>'
        for i, warning in enumerate(wiring["warnings"])
    )
    description = "; ".join(f'{item["from"]} to {item["to"]}' for item in connections)

    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 890" role="img" aria-labelledby="title desc">
