...
void unavailable(DashboardScreen screen){char top[17]{},bottom[17]{};switch(screen){case DashboardScreen::Clock:std::snprintf(top,sizeof(top),"%-16s","NTP Desk Clock");std::snprintf(bottom,sizeof(bottom),"%-16s","Time unavailable");break;case DashboardScreen::Mqtt:formatMqttText("MQTT Home","Waiting for data",top,bottom);break;case DashboardScreen::Unifi:formatUnifi(false,-1,-1,-1,top,bottom);break;case DashboardScreen::Satellite:formatSatellite(NAN,NAN,-1,top,bottom);break;case DashboardScreen::Weather:formatWeather(NAN,-1,nullptr,top,bottom);break;}storeFrame(screen,top,bottom);}
void initializeFrames(){for(uint8_t i=0U;i<5U;++i)unavailable(static_cast<DashboardScreen>(i));}
...
bool fetchUnifi(const DashboardConfig& value){if(!ensureTime(value.timezone))return false;char body[kMaximumResponse+1U]{};if(!fetchUrl(value.sources.unifi_url,body,sizeof(body)))return false;JsonDocument d;if(deserializeJson(d,body)!=DeserializationError::Ok)return false;JsonObject root=d.as<JsonObject>();if(root.isNull()||root.size()!=5U||!root["wan_up"].is<bool>()||!root["latency_ms"].is<int>()||!root["clients"].is<int>()||!root["access_points"].is<int>()||!root["observed_at"].is<long long>())return false;const int latency=d["latency_ms"].as<int>(),clients=d["clients"].as<int>(),aps=d["access_points"].as<int>();if(latency<0||latency>60000||clients<0||clients>100000||aps<0||aps>10000||!fresh(d["observed_at"].as<long long>(),120LL))return false;char top[17]{},bottom[17]{};formatUnifi(d["wan_up"].as<bool>(),latency,clients,aps,top,bottom);storeFrame(DashboardScreen::Unifi,top,bottom);return true;}
...
bool validateCandidate(const DashboardConfig& candidate){if(!connectWifi(candidate,15000U)||!synchronizeFresh(candidate.timezone))return false;BoundedMqttClient test;const bool mqtt_ok=test.connect(candidate.sources,mqttCallback);test.stop();return mqtt_ok && fetchUnifi(candidate) && fetchWeather(candidate);}
void refreshWorker(void* raw){auto* job=static_cast<RefreshJob*>(raw);runtime_refresh_active=true;bool ok=false;if(job->screen==DashboardScreen::Weather)ok=fetchWeather(active_config);else if(job->screen==DashboardScreen::Unifi)ok=fetchUnifi(active_config);else if(job->screen==DashboardScreen::Satellite)ok=fetchSatellite(active_config);runtime_refresh_active=false;if(ok&&timed_out_screen_marker==static_cast<uint8_t>(job->screen)+1U)timed_out_screen_marker=0U;if(!ok)unavailable(job->screen);if(xQueueSend(job->completed,&ok,0U)!=pdTRUE){Serial.println("Dashboard refresh completion queue failed");delay(50);ESP.restart();}vTaskSuspend(nullptr);}
...
void serviceRefresh(uint32_t now){if(refresh_task_handle!=nullptr){bool complete=false;if(xQueueReceive(refresh_completed,&complete,0U)==pdTRUE){vTaskDelete(refresh_task_handle);refresh_task_handle=nullptr;}return;}if(WiFi.status()!=WL_CONNECTED)return;DashboardScreen due{};bool selected=false;if(refreshAllowed(DashboardScreen::Weather,now)&&static_cast<uint32_t>(now-last_weather_ms)>=600000U){last_weather_ms=now;due=DashboardScreen::Weather;selected=true;}else if(refreshAllowed(DashboardScreen::Unifi,now)&&static_cast<uint32_t>(now-last_unifi_ms)>=30000U){last_unifi_ms=now;due=DashboardScreen::Unifi;selected=true;}else if(refreshAllowed(DashboardScreen::Satellite,now)&&static_cast<uint32_t>(now-last_satellite_ms)>=10000U){last_satellite_ms=now;due=DashboardScreen::Satellite;selected=true;}if(selected&&!startRefresh(due))unavailable(due);}
