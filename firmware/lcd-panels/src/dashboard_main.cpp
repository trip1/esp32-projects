...
void unavailable(DashboardScreen screen){char top[17]{},bottom[17]{};switch(screen){case DashboardScreen::Clock:formatMqttText("NTP Desk Clock","Time unavailable",top,bottom);break;case DashboardScreen::Satellite:formatSatellite(NAN,NAN,-1,top,bottom);break;case DashboardScreen::Weather:formatWeather(NAN,-1,nullptr,top,bottom);break;case DashboardScreen::Launch:formatMqttText("Next Launch","Unavailable",top,bottom);break;case DashboardScreen::Moon:formatMqttText("Moon","Unavailable",top,bottom);break;case DashboardScreen::Solar:formatMqttText("Solar Activity","Unavailable",top,bottom);break;case DashboardScreen::Planet:formatMqttText("Planet View","Unavailable",top,bottom);break;case DashboardScreen::Neo:formatMqttText("Near-Earth Obj","Unavailable",top,bottom);break;case DashboardScreen::DeepSpace:formatMqttText("Deep Space","Unavailable",top,bottom);break;}storeFrame(screen,top,bottom);}
void initializeFrames(){for(uint8_t i=0U;i<kDashboardScreenCount;++i)unavailable(static_cast<DashboardScreen>(i));}
void showImmediate(const char* top_text,const char* bottom_text){if(!lcd_ready)return;char top[17]{},bottom[17]{};formatMqttText(top_text,bottom_text,top,bottom);if(!lcd.show(top,bottom))lcd_ready=false;}
...
bool validateCandidate(const DashboardConfig& candidate){return connectWifi(candidate,15000U)&&synchronizeFresh(candidate.timezone)&&fetchWeather(candidate);}
...
bool startProvisioningVisible(){if(!dashboardStartProvisioning())return false;showImmediate("Setup ready","See USB serial");return true;}
void serviceProvisioningDisplay(uint32_t now){if(lcd_ready||static_cast<uint32_t>(now-last_lcd_attempt_ms)<5000U)return;last_lcd_attempt_ms=now;lcd_ready=lcd.begin();if(lcd_ready)showImmediate("Setup ready","See USB serial");}
}  // namespace

void setup(){
    Serial.begin(115200);initializeFrames();lcd_ready=lcd.begin();showImmediate("Smart Dashboard","Starting...");pinMode(SETUP_BUTTON_PIN,INPUT_PULLUP);
    active_ready=dashboardLoadConfig("active",active_config);DashboardConfig pending{};const bool pending_valid=dashboardLoadConfig("pending",pending);
    const bool pending_matches=pending_valid&&active_ready&&std::memcmp(&pending,&active_config,sizeof(pending))==0;
    const bool pending_ready=pending_valid&&!pending_matches&&!dashboardPendingSuppressed();
    if(!pending_valid)dashboardRemoveConfig("pending");else if(pending_matches)dashboardRemoveConfig("pending");
    if((active_ready||pending_ready)&&dashboardRecoveryRequested()){if(!startProvisioningVisible()){delay(30000);ESP.restart();}return;}
    if(pending_ready){
        dashboardBeginPendingValidation();showImmediate("Testing settings","Please wait");
        if(validateCandidate(pending)&&dashboardPromotePending(pending)){active_config=pending;active_ready=true;Serial.println("Dashboard settings verified and promoted");}
        else{dashboardRejectPending();WiFi.disconnect(true,false);showImmediate("Setup rejected","Reconnect setup");Serial.println("Dashboard settings rejected; active settings preserved");}
    }
    if(!active_ready){if(!startProvisioningVisible()){delay(30000);ESP.restart();}return;}
    if(WiFi.status()!=WL_CONNECTED){showImmediate("Connecting WiFi","Please wait");connectWifi(active_config,15000U);}
    configTzTime(active_config.timezone,"pool.ntp.org","time.nist.gov");
    refresh_completed=xQueueCreate(1U,sizeof(bool));if(refresh_completed==nullptr){showImmediate("Dashboard error","Restarting");Serial.println("Dashboard refresh queue allocation failed");delay(30000);ESP.restart();return;}
    const uint32_t ready_ms=millis();suppression_started_ms=ready_ms;last_weather_ms=ready_ms-600000U;last_space_summary_ms=ready_ms-300000U;last_satellite_ms=ready_ms-10000U;slot_started_ms=ready_ms;updateClock();show();
}

void loop(){
    if(dashboardProvisioningActive()){serviceProvisioningDisplay(millis());dashboardHandleProvisioning();delay(2);return;}
    const uint32_t now=millis();if(WiFi.status()!=WL_CONNECTED&&static_cast<uint32_t>(now-last_wifi_attempt_ms)>=30000U){last_wifi_attempt_ms=now;WiFi.begin(active_config.sources.wifi_ssid,active_config.sources.wifi_password);}
    if(static_cast<uint32_t>(now-last_clock_ms)>=1000U){last_clock_ms=now;updateClock();}
    serviceRefresh(millis());const uint32_t current=millis();if(dashboardSlotExpired(slot_started_ms,current,dashboardDurationForSlot(active_config.schedule,active_slot))){active_slot=dashboardNextSlot(active_config.schedule,active_slot);slot_started_ms=current;}show();delay(50);
}
