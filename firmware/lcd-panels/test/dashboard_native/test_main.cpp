...
int main() {
    ...
    schedule.order[2] = DashboardScreen::Satellite;
    schedule.order[3] = DashboardScreen::Weather;
    ...
    assert(dashboardNextSlot(schedule, 3U) == 0U);
    assert(dashboardDurationForSlot(schedule, 1U) == 20U);
    schedule.order[0] = DashboardScreen::Weather;
    schedule.order[3] = DashboardScreen::Clock;
    assert(dashboardDurationForSlot(schedule, 0U) == 40U);
    assert(dashboardDurationForSlot(schedule, 3U) == 10U);
    schedule.order[0] = DashboardScreen::Clock;
    schedule.order[3] = DashboardScreen::Weather;
